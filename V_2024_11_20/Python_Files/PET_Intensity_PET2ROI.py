...
def calculate_average_values(mask_path, nii_dir, output_csv, Prefix, roi_info=None, startswith=''):
    """
    ...
    with open(output_csv, mode='w', newline='') as file:
        ...
        for nii_filename in tqdm(nii_files, desc="Processing NIfTI Files", unit="file"):
            ...
            if not write_header:
                ...
                assert len(roi_info) >= len(unique_values)
...
# 使用示例
mask_path = r'AAL3v1_1mm.nii'  # ROI文件路径，不是目录
PET = r'testPET'  # PET2MRI,PET2ROI的文件目录
...
# 加载ROI信息
# 计算NIfTI文件的平均值并写入CSV
calculate_average_values(mask_path, PET, output_csv, Prefix, ROI_INFO, startswith=Prefix)
