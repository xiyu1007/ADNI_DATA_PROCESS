...
def Dcm2Nii(DICOM_Root, Nii_Root, Modality,niiCopy=True,del_er=True, verbose_mode= False, only_disp_err=True,NiiInAll=True):
    ...
    for modality in Modality:
        ...
        for patient_folder in tqdm(os.listdir(modality_path), desc=f"{modality} Dicom To Nii ...", unit="file"):
            ...
            if os.path.isdir(input_root):
                ...
                ex_Nii = [entry for entry in os.scandir(input_root)
                          if entry.is_file() and entry.name.endswith('.nii')]
                if ex_Nii and niiCopy:
                    src_path = ex_Nii[0].path  # 直接获取文件的绝对路径
                    dst_path = os.path.join(output_root, patient_folder + '.nii')  # 设置目标路径
                    shutil.copy(src_path, dst_path)  # 执行复制操作
                    continue
                # 构造 dcm2niix 命令
                command = [
                ...
                try:
                    ...
                    if not only_disp_err:
                        ...
                        if verbose_mode:
                            ...
                            print(
                                f"\nConverted dcm/nii files {str(patient_folder)} to NIfTI format.")
    ...
    if failed_conversions and del_er:
        for patient_folder, m in failed_conversions:
            ...
            for m in Modality:
                if NiiInAll:
                    ...
                    nii_to_delete = os.path.join(Nii_Root, m, patient_folder)
                    if os.path.exists(nii_to_delete):
