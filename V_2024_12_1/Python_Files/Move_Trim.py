...
def Move_Trim(ID,Modality, root_directory, target_root, csv_file_path, move_miss_path, endswith, moveNocopy):
    # 读取CSV文件
    ...
    for ii, row in tqdm(df.iterrows(), desc="Moving & Trimming files...", unit="file",total=len(df)):
        ...
        if not os.path.exists(dicom_root_path):
            ...
            #       {dicom_root_path})
            print(f"Not such path, {subject}, {modality}, {dicom_root_path}")
